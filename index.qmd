...
<img src = "images/donkey.png" height = 250 id = "donkey">
...
sdat %>%
  select(c(mtime, hr, sbp, dbp, temp, spO2, resp)) %>%
  reshape2::melt(id.vars = "mtime") %>%
  ggplot(aes(x = mtime, y = value)) +
  geom_point(size = 0.1, na.rm = TRUE) +
  facet_wrap("variable", scales = "free_y",
             labeller = as_labeller(c("hr" = "heart rate",
                                      "sbp" = "systolic blood pressure",
                                      "dbp" = "diastolic blood pressure",
                                      "temp" = "body temperature",
                                      "resp" = "respiratory rate",
                                      "spO2" = "oxygen saturation"))) +
  scale_x_continuous("follow-up time (hours)", breaks = seq(0, 200, 24)) +
  theme(strip.text = element_text(face = "bold", size = 10),
        plot.margin = margin(10, 10, 30, 10, unit = "pt"))

ggsave("images/pat1111.png", width = 8, height = 4.5, dpi = 600)

```


## {background-image="images/pat1111.png" background-color="$theme_bg" background-size="contain"}
...
Approx. 1000 patients at the general nursing ward.

<br>

* repeated measurements of 6 vital parameters
...
* measurements every 20 seconds
  ...
  * 67 - 108551 obs./patient (per parameter)
:::

:::{.fragment}
&#8680; 9.3 million rows

&#8680; 32.6 million observations
:::


# The Linear Mixed Model {background-image="images/mixture_colors.jpg" background-size="cover" background-color="black" background-opacity=0.6 .center id="MMsection"}
...
b_{i0}\\b_{i1}
\end{bmatrix} \sim N\left( \begin{bmatrix}0\\0\end{bmatrix}, \begin{bmatrix}d_{11} & d_{12}\\d_{12} & d_{22}\end{bmatrix}\right)
$$
...
# Scalability {background-image="images/mandelbrot.jpg" background-size="cover" background-color="black" background-opacity=0.5 .center id="scalability"}
...
data.frame(N = c(1000, 10000, 100000, 1e6, 10e6),
  ...
  column_spec(6, color = lighten(nord10, 0.2))
...
pl0 +
  scale_x_continuous("number of random effects terms",
                     breaks = seq(0, 250, 20)) +
  geom_line(aes(y = pars2), linetype = 2, color = nord4)
...
[**Scenario 1 & 2: Many clusters/patients or many repeated measurements**]{.blue}
...
:::{.fragment}
[**Scenario 3: Many covariates**]{.blue}
...
## Conclusion {id="conclusion4"}

<br>

[**Scenario 4: Many outcome variables**]{.blue}
...
* [stacking the data](#datastacking) & use interaction with outcome indicator
...
[**Scenario 5: Random Effect Terms**]{.blue}

* feasible for moderate number of random effects terms (e.g., 10) and 
...
data.frame(N = 1000,
  ...
  kable(format = "html", table.attr = "class='datatablarge'") %>%
...
# More Challenges!<br>(And Solutions?) {background-image="images/balancing.jpg" background-size="cover" background-color="black" background-opacity=0.8 .center id="challenges"}



## Dynamic Prediction{background-image="images/dynpred_hr_7703.gif" background-size="contain" background-color="white"}
...
[**Empirical Bayes**]{.blue} estimate:

$$\mathbf{\hat b}_i = \widehat{\mathbf D} \mathbf Z_i^\top 
\color{var(--nord11)}{\left(\mathbf Z_i \mathbf{\widehat D} \mathbf Z_i^\top + \sigma^2 \mathbf I\right)^{-1}} (\mathbf y_i - \mathbf x_i^\top \boldsymbol{\hat\beta})$$

$\mathbf Z_i$ may be (very) large, e.g. 20 000 rows

&#8680; $\mathbf Z_i \mathbf{\widehat D} \mathbf Z_i^\top$ is also very large!!!
...
Re-calculate $\mathbf{\hat b}_i$ repeatedly when new data becomes available, say,
every 5 minutes.
...
Update $\mathbf{\hat b}_i$ using the previous estimate:

$$
\mathbf{\hat b_i^{(j)}} = \mathbf{\hat b_i^{(j-1)}} + \mathbf K_i^{(j)} \left(\color{var(--nord14)}{\mathbf{\tilde X}_i^{(j)}}\boldsymbol{\hat\beta} - 
\color{var(--nord14)}{\mathbf{\tilde Z}_i^{(j)}} \mathbf{\hat b_i^{(k-1)}}\right) 
$$

where $\mathbf{\widetilde X}_{i}^{(j)}$ and $\mathbf{\widetilde Z}_{i}^{(j)}$ 
contain only [the new observations]{.green}, i.e., are small!

<br>

:::{.fragment}
:::{.fbox}
**Reduction of computation time from 3 months to 7 minutes!!!** (in Julia)
:::
:::
...
:::{.sgrey}
$$
\mathbf K^{(j)} = \mathbf{\widehat D}^{(j)} \mathbf{\widetilde Z}_{i}^{(j)\top} 
\left(\mathbf{\widetilde Z}_{i}^{(j)} \mathbf{\widehat D}^{(j)} \mathbf{\widetilde Z}_{i}^{(j)\top} +
\hat{\sigma}^2
\mathbf I\right)^{-1}
$$
$$
\mathbf{\widehat D}^{(j)} = \left(\mathbf I - \mathbf K^{(j-1)} \mathbf Z_i^{(j-1)}\right)\, \mathbf{\widehat D}^{(j-1)}
$$
:::
...
* Generalized Linear Mixed Model (like GLM vs LM)
* no closed-form solution for integrating out random effects
...
## Stacking the Data {id="datastacking"}
...
[{{<fa solid arrow-left >}}](#conclusion4)

## Pairwise Approach {id="pairwise"}

Fit models for all pairs of outcomes:
$$(y_1, y_2), (y_1, y_3), (y_2, y_3), \ldots, (y_{K-1}, y_K)$$

* average parameters estimated in multiple models<br>
...
::::{.columns}
:::{.column width="50%"}
[{{< fa regular smile >}}]{.green} Bivariate models can be run in parallel.
:::

:::{.column width="50%"}
[{{< fa regular frown >}}]{.red} Doesn't work in joint models.
:::
::::
:::
...
# Thank You for Your Attention! {.h1section .the-end .center .middle .no-footer}
